"""Round 4: Phase 4 per-chunk-per-language calls instead of all-10-langs in one call.
...
Saves to benchmark/phase4_qwen_round4_perlang/.
"""
...
BENCH_DIR = REPO_ROOT / "benchmark" / "phase4_qwen_round4_perlang"
...
async def call_qwen(client, system, user, schema, max_tokens=2048) -> CallResult:
    start = time.time()
    try:
...
async def translate_one_verse(client, sem, verse: dict, baseline_record: dict) -> dict:
    ...
    async def do_chunk_lang(chunk_idx, lang):
        async with sem:
            ...
            return ("chunk", chunk_idx, lang, await call_qwen(client, SYSTEM_PROMPT, user, chunk_schema, max_tokens=2048))

    async def do_meta_lang(lang):
        async with sem:
            user = build_meta_user(en_block.get("summary", ""), en_block.get("seo_question", ""),
                                   lang, LANG_NAMES[lang])
            return ("meta", None, lang, await call_qwen(client, SYSTEM_PROMPT, user, meta_schema, max_tokens=1024))
    ...
    return {
        ...
        "round": 4,
        "approach": "per-chunk-per-language",
...
async def main_async() -> None:
    ...
    logger.info("round 4: per-chunk-per-language calls, workers=%d", WORKERS)
