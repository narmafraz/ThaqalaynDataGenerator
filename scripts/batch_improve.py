...
IMPORTANT v4 CONTEXT:
- The pipeline now uses v4 format by default: model outputs `word_tags` ([word, POS] pairs)
  instead of full `word_analysis` with 11-language translations per word.
- `translations.*.text` is NOT generated by the model — it's reconstructed from chunk translations.
- `similar_content_hints` field has been removed from the prompt.
- Validation accepts both v3 (word_analysis) and v4 (word_tags) formats.
- The v4 word dictionary (`word_translations_dict_v4.json`) translates unique words corpus-wide.
- See V4_PIPELINE_PLAN.md for full architecture details.
...
- app/pipeline_cli/word_dictionary.py — corpus-wide word dictionary extraction and assembly
