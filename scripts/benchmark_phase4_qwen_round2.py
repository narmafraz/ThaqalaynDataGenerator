"""Round 2 Phase 4 Qwen benchmark — adds strict JSON-schema response_format.

Uses the same sample.json as round 1, but passes
  response_format = {"type": "json_schema", "json_schema": {...}}
which vLLM enforces during decode. This eliminates the malformation failures
seen in round 1 (duplicate "translations" keys, structural drift on long
verses, premature termination).

Saves to benchmark/phase4_qwen_round2/ so round 1 stays intact for comparison.
"""
...
logger = logging.getLogger("phase4_qwen_r2")
...
ROUND1_DIR = REPO_ROOT / "benchmark" / "phase4_qwen"
BENCH_DIR = REPO_ROOT / "benchmark" / "phase4_qwen_round2"
SAMPLE_PATH = ROUND1_DIR / "sample.json"  # reuse round 1 sample
...
def build_schema(num_chunks: int, include_metadata: bool) -> dict:
    """Build a strict JSON schema matching the prompt's expected structure."""
    lang_obj = {
        "type": "object",
...
async def translate_one_verse(client, verse: dict, baseline_record: dict) -> dict:
    ...
    for batch_idx, batch_chunks in enumerate(batches):
        ...
        cr = await call_qwen(client, system, user, schema, max_tokens=8192)
...
async def main_async() -> None:
    ...
    logger.info("ROUND 2: running %d verses against %s @ %s, workers=%d, response_format=json_schema",
