...
1. **Generate (Pass 1)** — `ai-generate` agent (Opus): Extracts verse data, builds prompt, generates all 13 fields, validates schema. For long hadiths (>200 words), uses chunked processing.
...
The pipeline generates 12 fields per hadith/verse (v4 removed `similar_content_hints`):

**v4 changes from v2.0:**
- **Field 4**: `word_analysis` → `word_tags` (just `[word, POS]` pairs, no per-word translations). Full `word_analysis` is reconstructed from `word_tags` + corpus-wide word dictionary.
- **Field 9**: `translations.*.text` no longer generated by model — reconstructed from chunk translations in `reconstruct_fields()`.
- **Field 13**: `similar_content_hints` removed (low value, not used downstream).
...
**Word dictionary** (`ai-pipeline-data/word_translations_dict_v4.json`):
- Corpus-wide `(word, POS)` → 11-language translations. ~100K-150K unique pairs at full corpus.
- Module: `app/pipeline_cli/word_dictionary.py` — extract, translate, assemble functions.
- One-time cost: ~$60-120 (Haiku) or ~$200-400 (Sonnet) for the entire dictionary.

**Supporting data files** (in `ThaqalaynDataSources/ai-pipeline-data/`):
- `topic_taxonomy.json` — Two-level taxonomy: 14 Level 1 tags × ~5-8 Level 2 topics each (~90 total)
- `key_phrases_dictionary.json` — Seed dictionary of ~160 common Islamic multi-word expressions
...
**Cache invalidation**: `PIPELINE_VERSION` bumped to `"4.0.0"`, which invalidates all cached results.
